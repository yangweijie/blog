var Page = (function() {
    ...
    function init() {
        ...
        $('div > :header',$bookBlock).each(function(i,v){
            $(v).parents('.bb-item').attr('id','chapter_'+i);
            $('#menu-toc').append('<li><a class="'+ $(v)[0].tagName.toLowerCase() +'" href="#chapter_'+i+'">'+$(v).text()+'</a></li>');
        })
        $('#menu-toc li:first').addClass('menu-toc-current');
        $menuItems = $container.find( 'ul.J_Menu > li' );
        initEvents();
        jump(hash);
    }
