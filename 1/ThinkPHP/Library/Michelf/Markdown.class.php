...
class Markdown {
	...
	protected function _doCodeBlocks_callback($matches) {
		...
		$codeblock = "<pre class='lang'><code>$codeblock\n</code></pre>";
	...
	/**
 	 * 为文字加颜色
	 */
	protected function doTextColor($text){
		...
		if(preg_match('/{(\w):(.+?)}/',$text,$matches) ){
			...
			$token = $matches[2];
			$span= "<span $c>$token</span>";
			$span= $this->hashPart($span);
			$token = $parts[0].$span;
			if (!preg_match('/{(\w):(.+?)}/',$parts[1],$matches)  ) {
				return $token.$parts[1];
			} else {
				return $token.$this->runSpanGamut($parts[1]);
			}
		} else {
			return $text;
		}
...
class _MarkdownExtra_TmpImpl extends \Michelf\Markdown {
	...
	#
		...
		foreach ($grafs as $key => $value) {
			...
			if ($is_p) {
				//根据段落最前面的！号的数量，为<p>标签增加相应的颜色css class
				$c = '';
				if ( preg_match( '/^(!{1,5})/', $value, $matches ) ) {
					...
					$c = "class='$c'";
					$value = trim(substr( $value, $length ));
				}
				//判断是否标题或版权和转载声明

				if(preg_match('/^\s*{(title):(.+)}\s*$/i',$value,$matches))
				{
					$c = "id='doc-{$matches[1]}'";
					$value = "<div $c>{$matches[2]}</div>";
				} elseif(preg_match('/^\s*({(from):.*})\s*$/Ui',$value,$matches)){
					$c = "id='doc-{$matches[2]}'";
					$value = preg_replace( '/(?<=:)\s*(\S*)\s*(?=[,}])/U', "'$1'",addslashes($matches[1]) );
					$value = trim(json_encode( $value ),'"');
					$value = "<div $c>{$value}</div>";
				}
				else {
					$value = "<p $c>{$value}</p>";
				}
