...
class UpdateController extends AdminController {
	...
	public function autoUpdate($version, $step = 0){
		...
		switch ($step) {
			case 0:
				...
				$this->redirect(
					'autoUpdate',
					array('version'=>$version,'step'=>1),
					1,'正在关闭前台站点访问...,完成后自动转入下一步'
				);
				$this->display();
				break;
			case 1:
			...
			case 2:
				...
				foreach ($list as $key => $value) {
					...
					$i = $key+1;
					if($remote && is_file($update_file) && @file_put_contents($update_file, $remote))
						$result = "<td class='success'>　　√</td>";
					else
						$result = "<td class='error'>　　X</td>";
					$msg = "<tr><td>{$i}</td><td>{$update_file}</td>{$result}</tr>";
	...
	//手动升级
	public function manual($version, $step =1){
		$list = $this->getUpdateList($version);
		$readme = "/Update/{$version}/readme.txt";
		$this->assign('readme', $readme);
		$this->display();
		file_put_contents("./Update/{$version}/readme.txt", "#oneblog {$version}版手动升级指南".PHP_EOL.PHP_EOL.'# 步骤：'.PHP_EOL);
		ob_end_clean(); //清除输出缓存并且关闭缓存
		echo str_pad(' ',256);//输出256字节空格（在ie浏览器下要接受大于256个字节才会输出显示
		foreach ($list as $key => $value) {
			create_dir_or_files($value['path']);
			...
			if($remote &&
				is_file("$update_file") &&
				@file_put_contents("$update_file", $remote))
				$result = "<td class='success'>　　√</td>";
			else
				$result = "<td class='error'>　　X</td>";
			$msg = "<tr><td>{$i}</td><td>{$update_file}</td>{$result}</tr>";
			switch ($value['opt']) {
				case 'add': case 'modify':
					$opt = "将{$update_file} 复制到".dirname($target_file).'。';
					break;
				case 'del':
					$opt = "将{$target_file}手动删除。";
					break;
				case 'sql':
					$opt = "做好数据库备份工作后，在数据库管理软件或者phpmyadmin里执行{$update_file}文件。";
					break;
			}
			file_put_contents(".{$readme}", $i.'. '.$opt.PHP_EOL, FILE_APPEND);
			file_put_contents(".{$readme}", '最后别忘了后台版本更新和清掉Runtime目录', FILE_APPEND);
			echo "<script type=\"text/javascript\">showmsg(\"{$msg}\");</script>";
			flush();
			ob_flush();
			usleep(500000);
